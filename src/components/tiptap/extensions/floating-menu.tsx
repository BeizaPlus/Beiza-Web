...
type FloatingMenuComponent = ComponentType<any>;

const useFloatingMenuComponent = () => {
  const [FloatingComponent, setFloatingComponent] = useState<FloatingMenuComponent | null>(null);
  ...
  useEffect(() => {
    ...
    void import("@tiptap/react")
      .then((module) => {
        ...
        if ("FloatingMenu" in module && typeof module.FloatingMenu === "function") {
          setFloatingComponent(() => module.FloatingMenu as FloatingMenuComponent);
        } else {
          // eslint-disable-next-line no-console
          console.warn("[tiptap] FloatingMenu export not available; slash command menu disabled.");
          setFloatingComponent(null);
        }
      })
      .catch((error) => {
        // eslint-disable-next-line no-console
        console.error("[tiptap] Failed to load FloatingMenu", error);
        if (mounted) {
          setFloatingComponent(null);
        }
      });
...
export function TipTapFloatingMenu({ editor }: { editor: Editor }) {
  const FloatingMenu = useFloatingMenuComponent();
