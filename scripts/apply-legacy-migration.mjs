/**
 * Applies legacy family vault migrations via direct Postgres connection.
...
const migrationFiles = [
  "20260519T120000_legacy_family_vault.sql",
  "20260519T120100_legacy_circle_creator_select.sql",
];
...
async function main() {
  ...
  const [{ exists }] = await sql`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'public' AND table_name = 'family_circles'
    ) AS exists
  `;
  console.log("family_circles exists:", exists);
  await sql.end();
}
